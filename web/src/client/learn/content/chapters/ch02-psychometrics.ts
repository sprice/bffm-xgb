...
export const chapter02Psychometrics: Chapter = {
  ...
  content: `
    ...
    ${section(
      ...
      `
        ${paragraph(
          `After reverse-scoring, each domain score is just a mean. If a respondent's 10 Extraversion items (keyed) average to 3.8, then <code>ext_score = 3.8</code>. That's the ${abbr("raw-scale target", "The score on the original questionnaire scale before any normalization or percentile conversion.")}. Training uses these raw 1-5 domain means; conversion to ${abbr("percentiles", "Rank-like interpretations that place a score relative to a reference population.")} happens later.`,
        )}
        ${paragraph(
    ...
    ${section(
      ...
      `
        ${paragraph(
          `${abbr("Reliability", "How consistently a measure captures the same trait rather than fluctuating due to noise.")} asks whether a measure is consistent. ${abbr("Validity", "Whether a score supports the interpretation you want to make from it.")} asks whether the score means what you think it means. Related but different. A score can be perfectly consistent and still measure the wrong thing.`,
        )}
        ${paragraph(
          "Short forms are hard because every removed item cuts away information. Going from 10 items per domain to 4, reliability usually drops. The question: can smarter item selection and smarter scoring recover enough of what was lost?",
        )}
        ${table(
        ...
        ${callout(
          ...
          paragraph(
            `This codebase demonstrates reconstruction of full-scale scores, which doesn't by itself establish equal ${abbr("external validity", "Evidence that a score relates appropriately to outside outcomes or behaviors.")}, equal ${abbr("factor structure", "The pattern of underlying latent dimensions inferred from how items relate to each other.")}, or equal clinical interpretability. We return to that distinction throughout.`,
          ),
        )}
      `,
    )}
    ${section(
      ...
      `
        ...
        ${paragraph(
          `Currently these values are locked in <code>artifacts/ipip_bffm_norms.json</code>. Later stages refer to a stable <code>data_snapshot_id</code> derived from the norms file hash, so the norms aren't just descriptive statistics; they're part of the ${abbr("run identity", "The exact artifact fingerprint that distinguishes one reproducible pipeline state from another.")}.`,
        )}
      `,
    )}
    ${section(
